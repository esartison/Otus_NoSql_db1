...
## MongoDB ##
MongoDB — это документоориентированный NoSQL относящаяся к типу **CP** по CAP. 

В типовой конфигурации есть мастер экземпляр и есть read-only реплики. Запись может вестить только на мастер узле.

Согласованность стативится во главу угла. 
...
Можно настроить read-only реплики и в случае недоступности мастер экземпляра, данные могут быть получены с реплики(данные могут быть не совсем свежие). Тоесть **AP** тоже может подходить.
